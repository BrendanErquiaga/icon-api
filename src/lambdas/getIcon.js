...
exports.getIcon = async (event, context) => {
  ...
  if(listResponse === null) {
    ...
    let imageResponse = httpResponse(200, responseBody, true);
    imageResponse.headers = {
      'Content-Type': 'image/png'
    }
    imageResponse.isBase64Encoded = true;
...
async function FormatIconResponse(listResponse, queryParams) {
  if (queryParams && queryParams.list) {
    ...
    let imageIconResponse = await GetImageIcon(listResponse.Contents[0].Key);
    return imageIconResponse;//TODO improve this so it never sends an alternate
  }
}
